...
(defcustom ac-clang-compile-directory-name
  ".release"
...
(defcustom ac-clang-compile-commands
  'ac-clang-find-compile-commands
  "Function to locate compile_commands.json file"
  :group 'auto-complete
  :type 'function
  :safe 'functionp)
...
(defun ac-clang-filter-command (cmd)
  (let* ((parts (split-string cmd " "))
         (pred (lambda(part) (string/starts-with part "-I")))
         (flags (filter pred parts)))
    flags)
  )

(defun ac-clang-auto-cflags ()
  (if (file-exists-p (funcall-or-value ac-clang-compile-commands))
      (let* ((data (json-read-file (funcall-or-value ac-clang-compile-commands)))
             (found-command nil)
             (cflags nil))
        (cl-loop for x across data do
        ...
        (if found-command
            (progn
              (setq cflags (ac-clang-filter-command found-command))
...
(defun --ac-clang-async-mode-destroy()
  (remove-hook 'auto-complete-mode-hook 'ac-common-setup t)
  )
